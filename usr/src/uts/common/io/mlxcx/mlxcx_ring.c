    ...
    mlxcx_buffer_t *b0, const mlxcx_tx_ctx_t *ctx)
{
	...
	if (ctx->mtc_inline_hdrlen > 0) {
		...
		while (rem > 0) {
			...
			if (take > rem)
				take = rem;
			bcopy(ctx->mtc_inline_hdrs + off, (void *)seg, take);
			off += take;
			rem -= take;
